...
export default function TournamentListRow({ t, index, onClick }) {
  ...
  return (
    <div onClick={onClick} className="group px-5 py-4 hover:bg-slate-700/30 transition-colors animate-fade-in cursor-pointer">
      ...
      <div className="md:hidden space-y-2">
        ...
        {hasOrganizer && t.organizer_whatsapp && (
          <button onClick={openOrganizerWhatsApp}
            ...
            <WaIcon small /> הרשמה דרך {t.organizer_name}
          </button>
        )}
        {hasOrganizer && t.organizer_registration_url && (
          <a href={t.organizer_registration_url} target="_blank" rel="noopener noreferrer"
            onClick={(e) => e.stopPropagation()}
      ...
      {/* Desktop layout */}
      <div className="hidden md:grid tournament-list-grid gap-4 items-start">
        ...
        <div className="flex flex-col gap-1.5 items-stretch self-center">
          ...
          {hasOrganizer && t.organizer_whatsapp && (
            <button onClick={openOrganizerWhatsApp}
              className="flex items-center justify-center gap-1.5 bg-[#25D366]/15 hover:bg-[#25D366]/30 border border-[#25D366]/40 text-[#4ade80] font-bold py-1.5 px-3 rounded-xl text-[11px] whitespace-nowrap transition-all">
              <WaIcon small /> דרך {t.organizer_name}
            </button>
          )}
          {hasOrganizer && t.organizer_registration_url && (
            <a href={t.organizer_registration_url} target="_blank" rel="noopener noreferrer"
...
function WaIcon({ small }) {
  return (
    <svg viewBox="0 0 24 24" className={`${small ? 'w-3.5 h-3.5' : 'w-4 h-4'} fill-current shrink-0`} xmlns="http://www.w3.org/2000/svg">
