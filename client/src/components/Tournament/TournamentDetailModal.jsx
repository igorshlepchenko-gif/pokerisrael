...
export default function TournamentDetailModal({ tournament: t, onClose, brands = [] }) {
  ...
  // רישום כפול — דרך המארגן (Runner Runner וכו')
    ...
    >
      ...
      <div className="modal-panel-h relative w-full sm:max-w-lg flex flex-col bg-slate-800 border border-slate-700 rounded-t-3xl sm:rounded-2xl shadow-2xl animate-slide-up overflow-hidden">
        ...
        <div className="sticky bottom-0 shrink-0 bg-slate-800/95 backdrop-blur border-t border-slate-700 px-5 py-4 space-y-2">
          {/* כפתור ראשי — Joker Club עברו לקישור ישיר לאתר שלהם במקום וואטסאפ; שאר המועדונים ממשיכים בוואטסאפ דרך המארח */}
          {isJokerClub ? (
            lateRegClosed ? (
              <span className="flex items-center justify-center gap-2 w-full font-bold py-3 px-4 rounded-xl text-base shadow-lg bg-slate-700 text-slate-400 cursor-not-allowed">
                ⏳ ההרשמה נסגרה
              </span>
            ) : (
              <div>
                <div className="flex justify-center mb-1 text-2xl animate-bounce select-none pointer-events-none" aria-hidden="true">👇</div>
                <a
                  href={JOKER_CLUB_REGISTRATION_URL}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center justify-center gap-2 w-full font-bold py-3 px-4 rounded-xl transition-all duration-200 text-base
                    ...
                    hover:scale-[1.02] active:scale-95 animate-pulse-slow"
                >
                  🔗 הרשמה ישירה
                </a>
              </div>
          ...
          {hasOrganizer && t.organizer_registration_url && (
            <a
  ...
  return (
    <div className="flex gap-2">
      <a href={waze} target="_blank" rel="noopener noreferrer"
        className="flex-1 flex items-center justify-center gap-1.5 min-h-[44px] bg-[#33ccff]/15 hover:bg-[#33ccff]/30 border border-[#33ccff]/40 text-[#7fdcff] font-bold py-2 px-3 rounded-xl transition-all text-sm">
        🚗 נווט ב-Waze
      </a>
      <a href={gmaps} target="_blank" rel="noopener noreferrer"
        className="flex-1 flex items-center justify-center gap-1.5 min-h-[44px] bg-slate-700/60 hover:bg-slate-600 border border-slate-600 text-slate-200 font-bold py-2 px-3 rounded-xl transition-all text-sm">
        🗺️ Google Maps
      </a>
    </div>
  );
